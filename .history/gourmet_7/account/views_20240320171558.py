...
from .models import Restaurant, Photo, Event
...
def register_restaurant(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('restaurant_list')  # 成功した場合のリダイレクト先
    else:
        form = RestaurantForm()
    return render(request, 'account/register_restaurant.html', {'form': form})  # 適切な
